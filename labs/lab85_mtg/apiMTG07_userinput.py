...
def main():
    ...
    #cards = resp.json()
    user_cards = resp.json().get("cards")
    #open a file to save the data from user code selection
    with open("4DE_cards.index", "w") as fourdefile:
        for card in user_cards:
            print(f"{card.get('name')} | {card.get('type')}", file=fourdefile)
